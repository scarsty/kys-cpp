...
*/
    ...
    private static final int KYS_STORAGE_PERMISSION_REQUEST = 0x4B59;
    private static final int KYS_MANAGE_ALL_FILES_PERMISSION_REQUEST = 0x4B5A;
    ...
    private void requestStoragePermissionOnStartup() {
        if (Build.VERSION.SDK_INT >= 30 /* Android 11 (R) */) {
            if (!Environment.isExternalStorageManager()) {
                try {
                    Intent intent = new Intent(Settings.ACTION_MANAGE_APP_ALL_FILES_ACCESS_PERMISSION,
                            Uri.parse("package:" + getPackageName()));
                    startActivityForResult(intent, KYS_MANAGE_ALL_FILES_PERMISSION_REQUEST);
                } catch (Exception e) {
        ...
        if (Build.VERSION.SDK_INT >= 23 /* Android 6.0 (M) */) {
            ...
            if (!missingPermissions.isEmpty()) {
                requestPermissions(missingPermissions.toArray(new String[0]), KYS_STORAGE_PERMISSION_REQUEST);
            }
        }
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == KYS_MANAGE_ALL_FILES_PERMISSION_REQUEST && Build.VERSION.SDK_INT >= 30 /* Android 11 (R) */) {
            if (!Environment.isExternalStorageManager()) {
                Toast.makeText(this, "Storage permission is required for game data access.", Toast.LENGTH_LONG).show();
            }
            return;
        }
...
//        }
    ...
    public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        if (requestCode == KYS_STORAGE_PERMISSION_REQUEST) {
