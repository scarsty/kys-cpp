...
Hanz2Piny::toPinyinFromUnicode(const Unicode hanzi_unicode, const bool with_tone) const
{
    ...
    {
        auto iter = polyphone.cbegin();
        // 查找第一个数字（声调）的位置
        // 改为不使用 std::find_if，在不同编译器中写法不太一样
        while (iter != polyphone.cend() && !isdigit(*iter))
        {
            ++iter;
        }
...
{
    bool IsUTF8 = true;
    unsigned char* start = (unsigned char*)s.c_str();
    unsigned char* end = (unsigned char*)s.c_str() + s.size();
    while (start < end)
    {
...
int Hanz2Piny::guess_utf8_char_length(const std::string& s, const int pos) const
{
    ...
    {
        return 2;    // 2 字节 UTF-8 字符
    }
    else if ((c & 0xF0) == 0xE0)
    {
        return 3;    // 3 字节 UTF-8 字符
    }
    else if ((c & 0xF8) == 0xF0)
    {
        return 4;    // 4 字节 UTF-8 字符
    }
    return 1;    // 默认返回 1，表示可能是 ASCII 字符或非法字符，避免死循环
}

Hanz2Piny::Unicode Hanz2Piny::utf8_to_unicode(const std::string& s, const int pos) const
{
    if (pos < 0 || pos >= static_cast<int>(s.size()))
    {
        return 0;    // 无效位置
    ...
    {
        // 3 字节 UTF-8 字符
        if (pos + 2 >= static_cast<int>(s.size()))
        {
            return 0;    // 不足三个字节
        }
        return ((c & 0x0F) << 12) | ((static_cast<unsigned char>(s[pos + 1]) & 0x3F) << 6)
...
Hanz2Piny::toPinyinFromUtf8(const std::string& s, const bool with_tone, const bool replace_unknown, const std::string& replace_unknown_with) const
{
    ...
    for (auto iter = s.cbegin(); iter != s.cend(); NULL)
    {
        ...
        auto iter_old = iter;
        auto length = guess_utf8_char_length(s, iter - s.cbegin());
        Unicode unicode = utf8_to_unicode(s, iter - s.cbegin());
        iter += length;    // 移动到下一个字符
