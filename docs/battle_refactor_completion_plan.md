...
- Slice 7 Task 7.3: Open. The current checkpoint removed the worst `AttackEffect` gameplay bridge paths and keeps `BattleAttackWorld` as migration gameplay state, but user review rejected the earlier completion claim. `BattleSceneHades` still owns substantial battle decisions in frame runtime, hit resolution, combo side effects, team effects, targeting, and result/presentation coupling. Treat the current work as a boundary-reset checkpoint, not 7.3 completion.
