...
- Slice 4 Task 4.3: In progress. Core now owns projectile bounce target selection/spawn shape during migration; the scene no longer calls its duplicate bounce target selector after hit events, and the adapter appends core-spawned bounce attacks back into legacy `AttackEffect` playback state. Scene still applies legacy hit damage from core hit events until the damage transaction API lands in Slice 5.
