...
- Slice 5 Task 5.3: In progress. Scripted hit status handling now goes through `BattleDamageSystem::resolveTransaction()` with accepted zero-damage hits, including freeze resistance/control-immunity inputs derived from combo state. The delayed `HurtThisFrame` HP-damage/death loop now applies pre-resolved damage through `resolveTransaction()` for death prevention and kill rewards. Remaining work: migrate legacy hit-side resource/status/cooldown branches and the final HP/MP damage queuing out of `applyLegacyMagicHitTransaction()`.
