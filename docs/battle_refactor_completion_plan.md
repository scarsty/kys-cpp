...
- Slice 7 Task 7.3: Superseded/delegated. This document records the historical Slice 7.3 path and checkpoint evidence, but it is no longer the actionable finish-line plan. The earlier backend-exit completion claim was rejected after source review, and the first continuation direction was rejected after it drifted toward moving gameplay helpers into the adapter. Continue remaining `BattleSceneHades` backend-exit work from `docs/battle_scene_hades_backend_exit_continuation_reset_plan.md`.
...
> **Superseded/delegated:** This historical 7.3 task is no longer the executable finish-line plan. The first backend-exit finish-line plan and first continuation plan are historical because they allowed completion claims without a real one-frame state owner. Use `docs/battle_scene_hades_backend_exit_continuation_reset_plan.md` for the remaining work from the current branch state. Keep this section only as context and checkpoint history.
