...
- Debug `.github/build-command.ps1` linked `kys`, `kys_tests`, and `kys_chess_cli`; the complete Debug suite passed **8,927 assertions in 702 test cases**.
- Release `.github/build-command.ps1 -Configuration Release` linked `kys`, `kys_tests`, and `kys_chess_cli`; after a clean `kys_tests:Rebuild` removed stale incompatible artifacts, the complete Release suite passed **8,927 assertions in 702 test cases**.
...
- The `showmethemoney` regression passed **21 assertions in 1 test case**: the legacy `+100` changes only session money, preserves the complete journal prefix/chain and full RNG state, immediately fails explicit verification at the footer, and fails at the next accepted action's pre-state while normal action legality uses the cheated balance.
- The final dependency/source-scan group passed **820 assertions in 8 test cases**.
- The default graphical Debug CMake preset built the final GUI sources, and default CTest passed **702/702** tests.
...
- Normal graphical adapters submit `ChessAction` values and consume session transitions/observations. The sole intentional exception is the exact `showmethemoney` console code, which calls a session-owned stable-boundary cheat method; GUI code never receives mutable session state. `graphical adapters do not call gameplay mutation owners` found no direct GUI-owned rule mutation.
...
7. Graphical decisions route through `ChessGameSession` — `GUI adapter and direct session calls produce identical hashes` plus `graphical adapters do not call gameplay mutation owners` cover routing and prohibit direct mutation. The explicit `showmethemoney` cheat also remains session-owned rather than exposing mutable state to Console.
...
9. Every accepted decision is journaled automatically — `accepted management actions journal from sequence one`, replay round-trip tests, and the scripted-run verifier coverage prove automatic records from the first accepted action. The deliberate cheat API is not a `ChessAction` or accepted gameplay decision and is tested to leave the journal unchanged.
...
20. Windows x64 MSVC v145 Debug and Release accept the same committed goldens — both complete suites passed **8,927 assertions in 702 cases**, including `committed short and complete replay goldens verify`.
...
- The GUI adapter includes `SuperMenuText`, role previews, roster/combo summaries, detailed chess, combo, internal-skill, equipment, challenge, experience, formation, reroll, guide, and system-settings panels, plus graphical pre/post-battle lineup and statistics screens; normal gameplay mutations remain routed through `ChessGameSession` actions.
- The exact lowercase `showmethemoney` console code again grants the legacy 100 gold through `ChessGameSession::grantUnjournaledCheatMoney`. It is restricted to a stable decision boundary, consumes no RNG, writes no action/event/journal record, and leaves normal action legality enabled against the resulting balance.
