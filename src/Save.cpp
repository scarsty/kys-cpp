...
std::vector<Character> m_Character(config::MaxRole);
std::vector<BasicData> m_BasicData(1);
Calendar m_Calendar;
std::vector<Faction> m_Faction(config::Maxfaction);
std::vector<Item> m_Item(config::MAX_ITEM_AMOUNT);
std::vector<Magic> m_Magic(config::MaxMagic);
std::vector<ZhaoShi> m_ZhaoShi(config::MaxZhaoShi);
std::vector<BaoShop> m_Baoshop(10);
std::vector<SceneData> m_SceneData(config::MAXScene);
std::vector<SceneMapData> m_SceneMapData(config::MAXScene);
std::vector<SceneEventData> m_SceneEventData(config::MAXScene);
