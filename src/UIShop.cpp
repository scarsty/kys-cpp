...
void UIShop::draw()
{
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        str += format1::format("{:8}{:8}{:8}{:8}", shop_->Price[i], shop_->Total[i], count, plan_buy_[i]);
        //str = format1::format("{:12}{:8}{:8}{:8}{:8}", item->Name, shop_->Price[i], shop_->Total[i], count, plan_buy_[i]);
