...
BattleSceneHades::BattleSceneHades(int id) : BattleSceneHades() { setID(id); }
...
void BattleSceneHades::setID(int id) {
  ...
  BattleMap::getInstance()->copyLayerData(info_->BattleFieldID, 0,
                                          &earth_layer_);
  BattleMap::getInstance()->copyLayerData(info_->BattleFieldID, 1,
                                          &building_layer_);
}
...
void BattleSceneHades::draw() {
  ...
  Engine::getInstance()->fillColor({0, 0, 0, 255}, 0, 0, render_center_x_ * 2,
                                   render_center_y_ * 2);
  ...
  if (earth_texture_) {
    ...
    Engine::getInstance()->fillColor({0, 0, 0, 255}, 0, 0,
                                     COORD_COUNT * TILE_W * 2,
                                     COORD_COUNT * TILE_H * 2);
    for (int sum = -view_sum_region_; sum <= view_sum_region_ + 15; sum++) {
      for (int i = -view_width_region_; i <= view_width_region_; i++) {
        ...
        if (!isOutLine(ix, iy)) {
          ...
          bool need_draw = true;
          if (need_draw && num > 0) {
            TextureManager::getInstance()->renderTexture("smap", num, p.x,
                                                         p.y / 2, color);
          }
        }
      }
    ...
    for (auto r : battle_roles_) {
      ...
      info.shadow = 1;
      // TextureManager::getInstance()->renderTexture(path, pic, r->X1, r->Y1,
      // color, alpha);
      building_vec.emplace_back(std::move(info));
    }
    ...
    for (auto &ae : attack_effects_) {
      // for (auto r : ae.Defender)
      {
        ...
        info.alpha = 192;
        building_vec.emplace_back(std::move(info));
        // TextureManager::getInstance()->renderTexture(ae.Path, ae.Frame %
        // ae.TotalEffectFrame, ae.X1, ae.Y1 / 2, { 255, 255, 255, 255 }, 192);
      }
    }
    ...
    for (auto &d : building_vec) {
      ...
      TextureManager::getInstance()->renderTexture(d.path, d.num, d.p.x,
                                                   d.p.y / 2 - d.p.z, d.color,
                                                   d.alpha, scaley, 1, d.rot);
    }
    ...
    BP_Rect rect0 = {int(pos_.x - render_center_x_ - x_),
                     int(pos_.y / 2 - render_center_y_ - y_), w, h},
            rect1 = {0, 0, w, h};

    for (auto &te : text_effects_) {
      Font::getInstance()->draw(te.Text, 15, te.Pos.x, te.Pos.y / 2, te.Color,
                                255);
    }
...
void BattleSceneHades::dealEvent(BP_Event &e) {
  ...
  if (r->Dead == 0) {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (magic[i] && r->getMagicOfRoleIndex(magic[i]) < 0) {
        magic[i] = nullptr;
      }
    ...
    if (r->Frozen == 0 && r->CoolDown == 0) {
      int index = -1;
      if (engine->checkKeyPress(BPK_j)) {
        index = 0;
      }
      if (engine->checkKeyPress(BPK_i)) {
        index = 1;
      }
      if (engine->checkKeyPress(BPK_k)) {
        index = 2;
      }
      if (engine->checkKeyPress(BPK_m)) {
        index = 3;
      }
    ...
    for (auto r : battle_roles_) {
      ...
      if (r->ActType >= 0) {
        ...
        if (r->Acted == 0 &&
          ...
          r->PhysicalPower = GameUtil::limit(
              r->PhysicalPower - 3, 0, Role::getMaxValue()->PhysicalPower);
          int level_index = r->getMagicLevelIndex(magic->ID);
          int needMP = magic->calNeedMP(level_index);
          ae.TotalEffectFrame =
              TextureManager::getInstance()->getTextureGroupCount(ae.Path);
          ae.TotalFrame = 30;
          ...
          if (r->Team == 0) {
            ...
            if (r->OperationType == 2 && (ae.UsingMagic->AttackAreaType != 1 &&
                                          ae.UsingMagic->AttackAreaType != 2)) {
              ae.OperationType = -1;
            }
            ...
            if (ae.UsingMagic->AttackAreaType == 0) {
              ae.OperationType = 0;
            } else if (ae.UsingMagic->AttackAreaType == 1 ||
                       ae.UsingMagic->AttackAreaType == 2) {
              ae.OperationType = 2;
          ...
          }
          {
            ...
            if (index >= 0) {
              r->MagicLevel[index] = GameUtil::limit(
                  r->MagicLevel[index] + rand_.rand() * 2 + 1, 0, 999);
            }
            if (ae.OperationType == 0) {
              ...
              int range = magic->AttackDistance[level_index] +
                          magic->SelectDistance[level_index];
              int count = range;
              auto p = ae.Pos;
              ae.TotalFrame = 60;
              ...
              for (int i = 0; i < count; i++) {
                ae.Pos = p;
                ae.Speed = {cos(angle + i * 2 * M_PI / count),
                            sin(angle + i * 2 * M_PI / count)};
                ae.Speed.norm(3);
              ...
              auto p = ae.Pos;
              int count =
                  (r->Speed + r->getActProperty(ae.UsingMagic->MagicType)) / 60;
              for (int i = 0; i < count; i++) {
          ...
          //    attack_effects_.push_back(ae);
          //    int range = magic->AttackDistance[level_index] +
          //    magic->SelectDistance[level_index]; int count = range; int x1 =
          //    ae.X1; int y1 = ae.Y1; for (int i = 0; i < count; i++)
          //    {
        ...
        if (r->ActFrame >= r->FightFrame[r->ActType] - 2) {
          if (r->CoolDown == 0) {
            ...
            r->ActFrame =
                r->FightFrame[r->ActType] -
                2; // cooldown结束前停在最后行动帧，最后一帧一般是无行动的图
          }
        } else {
      ...
      //计算有人被打中
      for (auto &ae : attack_effects_) {
        if (!(r->OperationType == 2 && r->CoolDown) && !r->HurtFrame &&
            !r->Invincible && r->Dead == 0 && r->Team != ae.Attacker->Team &&
          ...
          fmt1::print("{} attack {} with {} as {}, hurt {}\n",
                      ae.Attacker->Name, r->Name, ae.UsingMagic->Name,
                      ae.OperationType, hurt);
          TextEffect te;
      ...
      if (r != role_ && r->Dead == 0) {
        if (r->CoolDown == 0) {
          ...
          if (r0) {
            ...
            r->FaceTowards = Towards_RightDown;
            if (r->RealTowards.x > 0 && r->RealTowards.y < 0) {
              r->FaceTowards = Towards_RightUp;
            }
            if (r->RealTowards.x < 0 && r->RealTowards.y > 0) {
              r->FaceTowards = Towards_LeftDown;
            ...
            if (EuclidDis(r->Pos, r0->Pos) > TILE_W * 3) {
              ...
              if (r->PhysicalPower >= 30) {
                ...
                for (int i = 1; i <= 4; i++) {
                  if (r->FightFrame[i] > 0) {
                    auto m = Save::getInstance()->getMagic(r->MagicID[0]);
                    if (m->AttackAreaType == 0) {
                      r->OperationType = 0;
                    } else if (m->AttackAreaType == 1 ||
                               m->AttackAreaType == 2) {
                      r->OperationType = 2;
                    } else if (m->AttackAreaType == 3) {
                      r->OperationType = 1;
                    }
                    r->CoolDown = calCoolDown(i, r->OperationType, r);
                    r->ActFrame = 0;
                    r->ActType = i;
                    break;
                  }
...
void BattleSceneHades::onEntrance() {
  ...
  head_boss_->setPosition(Engine::getInstance()->getWindowWidth() / 2 -
                              head_boss_->getWidth() / 2,
                          Engine::getInstance()->getWindowHeight() - 100);
  addChild(MainScene::getInstance()->getWeather());

  earth_texture_ = Engine::getInstance()->createARGBRenderedTexture(
      COORD_COUNT * TILE_W * 2, COORD_COUNT * TILE_H * 2);
  ...
  std::sort(enemies_.begin(), enemies_.end(), [](Role *l, Role *r) {
    return l->Level * 10000 + l->MaxHP < r->Level * 10000 + r->MaxHP;
  });
  if (!enemies_.empty()) {
    head_boss_->setRole(enemies_.back());
  }
  for (int i = 0; i < 6; i++) {
    if (!enemies_.empty()) {
      battle_roles_.push_back(enemies_.front());
  ...
  for (int i = 0; i < equip_magics_.size(); i++) {
    ...
    if (m && role_->getMagicOfRoleIndex(m) < 0) {
      m = nullptr;
    }
    if (m) {
...
void BattleSceneHades::renderExtraRoleInfo(Role *r, double x, double y) {
  ...
  Engine::getInstance()->renderSquareTexture(&r1, background_color,
                                             192 * alpha);
}
...
int BattleSceneHades::defaultMagicEffect(AttackEffect &ae, Role *r) {
  int hurt = calMagicHurt(ae.Attacker, r, ae.UsingMagic,
                          EuclidDis(r->Pos, -ae.Attacker->Pos) / TILE_W / 2);
  //这个击退好像效果不太对
  r->Velocity = r->Pos - ae.Attacker->Pos;
  r->Velocity.norm(1);
  r->SpeedFrame = 10;
...
void BattleSceneHades::makeSpecialMagicEffect() {
  special_magic_effect_["葵花神功"] = [this](AttackEffect &ae, Role *r) -> int {
    int hurt = defaultMagicEffect(ae, r);
    r->Frozen += 20; //定身
    r->MP -= hurt;   //消耗敌人內力
    return hurt;
  };
  special_magic_effect_["獨孤九劍"] = [this](AttackEffect &ae, Role *r) -> int {
    int hurt = defaultMagicEffect(ae, r);
    r->Frozen += 10; //定身
    r->ActType = -1; //行动打断
    r->OperationType = -1;
  ...
  special_magic_effect_["降龍十八掌"] = [this](AttackEffect &ae,
                                               Role *r) -> int {
    int hurt = defaultMagicEffect(ae, r);
