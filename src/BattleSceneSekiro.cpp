...
void BattleSceneSekiro::draw()
{
    ...
    drawClassicPresentation();
}

void BattleSceneSekiro::adjustClassicRoleDrawInfo(Role* role, ClassicDrawInfo& info)
{
    if (role->ActType < 0 || role->ActType > 4)
    {
        auto path = std::format("walk/walk{:03}", role->HeadID);
        int count_for_one = TextureManager::getInstance()->getTextureGroupCount(path) / 4;
        if (count_for_one > 0)
        {
            info.path = path;
            info.num = role->FaceTowards * count_for_one + (role->WalkingStep / 5) % count_for_one;
        }
    }
    if (role->Breathless && role->HP > 0)
    {
        info.breathless = 1;
        info.rot2 = role->CoolDown;
    }
}

void BattleSceneSekiro::drawClassicExtraEffects()
{
    if (sword_light_)
    {
