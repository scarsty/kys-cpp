...
template<typename Range, typename Value, typename Projection>
auto tryDenseBy(Range& range, const Value& value, Projection projection)
{
    using Reference = std::ranges::range_reference_t<Range>;
    using Element = std::remove_reference_t<Reference>;

    if (value < 0 || static_cast<std::size_t>(value) >= std::ranges::size(range))
    {
        return static_cast<Element*>(nullptr);
    }

    auto& item = range[static_cast<std::size_t>(value)];
    assert(std::invoke(projection, item) == value);
    return std::addressof(item);
}
...
template<typename Range, typename Value, typename Projection>
decltype(auto) requireDenseBy(Range& range, const Value& value, Projection projection)
{
    assert(value >= 0);
    assert(static_cast<std::size_t>(value) < std::ranges::size(range));
    auto& item = range[static_cast<std::size_t>(value)];
    assert(std::invoke(projection, item) == value);
    return item;
}
