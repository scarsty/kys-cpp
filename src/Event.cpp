...
bool Event::loadEventData() {
  //读取talk
  auto talk =
      GrpIdxFile::getIdxContent("../game/resource/talk.idx",
                                "../game/resource/talk.grp", &offset, &length);
  ...
  for (int i = 0; i < length.size(); i++) {
    std::string str = convert::replaceAllSubString(
        PotConv::cp950toutf8(talk.data() + offset[i]), "*", "");
    talk_contents_.push_back(str);
  }
  //读取事件，全部转为整型
  auto kdef =
      GrpIdxFile::getIdxContent("../game/resource/kdef.idx",
                                "../game/resource/kdef.grp", &offset, &length);
...
//返回值为是否成功执行事件
bool Event::callEvent(int event_id, RunNode *subscene, int supmap_id,
                      int item_id, int event_index, int x, int y) {
  bool ret = true;
  if (event_id <= 0 || event_id >= kdef_.size()) {
    return false;
  }
...
#define REGISTER_INSTRUCT(code, function)                                      \
  ...
  if (use_script_) {
    auto script =
        fmt1::format("../game/script/oldevent/oldevent_{}.lua", event_id);
    ...
    while (i < e.size() && !exit_) {
      ...
      case 50:
        if (e[i + 1] > 128) {
          fmt1::print("{}\n", "checkHave5Item");
          if (checkHave5Item(e[i + 1], e[i + 2], e[i + 3], e[i + 4],
                             e[i + 5])) {
            i += e[i + 6];
          } else {
            i += e[i + 7];
          ...
          instruct_50e(e[i + 1], e[i + 2], e[i + 3], e[i + 4], e[i + 5],
                       e[i + 6], e[i + 7], &e[i + 8]);
          i += 8;
        }
        break;
...
SubMapInfo *Event::getSubMapRecordFromID(int submap_id) {
  ...
  if (submap_record == nullptr) {
    submap_record = subscene_->getMapInfo();
  }
...
void Event::callLeaveEvent(Role *role) { callEvent(getLeaveEvent(role)); }
...
void Event::addItem(int item_id, int count) {
  ...
  text_box_->setText(fmt1::format(
      "獲得{}{}", Save::getInstance()->getItem(item_id)->Name, count));
...
//修改事件定义
void Event::modifyEvent(int submap_id, int event_index, int cannotWalk,
                        int index, int event1, int event2, int event3,
                        int currentPic, int endPic, int beginPic, int picDelay,
                        int x, int y) {
  if (submap_id < 0) {
...
//是否使用了某物品
bool Event::isUsingItem(int item_id) { return item_id_ == item_id; }
    ...
    if (r < 0) {
      ...
      for (int i = 0; i < ROLE_TAKING_ITEM_COUNT; i++) {
        if (role->TakingItem[i] >= 0) {
          if (role->TakingItemCount[i] == 0) {
            role->TakingItemCount[i] = 1;
          }
...
void Event::add3EventNum(int submap_id, int event_index, int v1, int v2,
                         int v3) {
...
void Event::setRoleMagic(int role_id, int magic_index_role, int magic_id,
                         int level) {
  auto r = Save::getInstance()->getRole(role_id);
...
void Event::addMorality(int value) {
  ...
  role->Morality =
      GameUtil::limit(role->Morality + value, 0, Role::getMaxValue()->Morality);
}

void Event::changeSubMapPic(int submap_id, int layer, int old_pic,
                            int new_pic) {
  auto s = getSubMapRecordFromID(submap_id);
  if (s) {
...
bool Event::checkFemaleInTeam() {
  for (auto r : Save::getInstance()->Team) {
    if (r >= 0) {
      if (Save::getInstance()->getRole(r)->Sexual == 1) {
        return true;
      }
    }
  }
...
void Event::play2Amination(int event_index1, int begin_pic1, int end_pic1,
...
void Event::play3Amination(int event_index1, int begin_pic1, int end_pic1,
                           int event_index2, int begin_pic2, int event_index3,
                           int begin_pic3) {
  auto e1 = subscene_->getMapInfo()->Event(event_index1);
  auto e2 = subscene_->getMapInfo()->Event(event_index2);
...
bool Event::checkHave5Item(int item_id1, int item_id2, int item_id3,
                           int item_id4, int item_id5) {
  return (haveItemBool(item_id1) && haveItemBool(item_id2) &&
          haveItemBool(item_id3) && haveItemBool(item_id4) &&
          haveItemBool(item_id5));
}

void Event::askSoftStar() { oldTalk(2547 + rand_.rand_int(18), 114, 0); }

void Event::showMorality() {
  text_box_->setText(fmt1::format("你的道德指數為{}",
                                  Save::getInstance()->getRole(0)->Morality));
  text_box_->run();
}

void Event::showFame() {
  text_box_->setText(
      fmt1::format("你的聲望指數為{}", Save::getInstance()->getRole(0)->Fame));
  text_box_->run();
}
...
void Event::fightForTop() {
  std::vector<int> heads = {8,  21, 23, 31, 32, 43, 7,  11, 14, 20,
                            33, 34, 10, 12, 19, 22, 56, 68, 13, 55,
                            62, 67, 70, 71, 26, 57, 60, 64, 3,  69};

  for (int i = 0; i < 15; i++) {
    int p = rand_.rand_int(2);
    oldTalk(2854 + i * 2 + p, heads[i * 2 + p],
            rand_.rand_int(2) * 4 + rand_.rand_int(2));
...
void Event::backHome(int event_index1, int begin_pic1, int end_pic1,
                     int event_index2, int begin_pic2, int end_pic2) {
  subscene_->forceManPic(-2);
  play2Amination(event_index1, begin_pic1, end_pic1, event_index2, begin_pic2,
                 end_pic2);
  RunNode::exitAll(1);
  forceExit();
}
...
void Event::playWave(int wave_id) { Audio::getInstance()->playASound(wave_id); }
...
void Event::instruct_50e(int code, int e1, int e2, int e3, int e4, int e5,
                         int e6, int *code_ptr) {
  ...
  case 16: //写存档数据
    ...
    switch (e2) {
    case 0:
      save_int_ptr = (int *)((char *)(save->getRole(e3)) + e4);
      break;
    case 1:
      save_int_ptr = (int *)((char *)(save->getItem(e3)) + e4);
      break;
    case 2:
      save_int_ptr = (int *)((char *)(save->getSubMapInfo(e3)) + e4);
      break;
    case 3:
      save_int_ptr = (int *)((char *)(save->getMagic(e3)) + e4);
      break;
    case 4:
      save_int_ptr = (int *)((char *)(save->getShop(e3)) + e4);
      break;
    }
    if (save_int_ptr) {
      *save_int_ptr = e5;
  ...
  case 17: //读存档数据
    ...
    switch (e2) {
    case 0:
      x50[e5] = *(int *)((char *)(save->getRole(e3)) + e4);
      break;
    case 1:
      x50[e5] = *(int *)((char *)(save->getItem(e3)) + e4);
      break;
    case 2:
      x50[e5] = *(int *)((char *)(save->getSubMapInfo(e3)) + e4);
      break;
    case 3:
      x50[e5] = *(int *)((char *)(save->getMagic(e3)) + e4);
      break;
    case 4:
      x50[e5] = *(int *)((char *)(save->getShop(e3)) + e4);
      break;
    }
    break;
  case 18: //写队伍数据
  ...
  case 27: //读名字到变量
    ...
    switch (e2) {
    case 0:
      sprintf(char_ptr, "%s", save->getRole(e3)->Name);
      break;
    case 1:
      sprintf(char_ptr, "%s", save->getItem(e3)->Name);
      break;
    case 2:
      sprintf(char_ptr, "%s", save->getSubMapInfo(e3)->Name);
      break;
    case 3:
      sprintf(char_ptr, "%s", save->getMagic(e3)->Name);
      break;
    }
    break;
  ...
  case 41: //画一张图
    ...
    switch (e2) {
    case 0:
      TextureManager::getInstance()->renderTexture("mmap", e5, e3, e4);
      break;
    case 1:
      TextureManager::getInstance()->renderTexture("head", e5, e3, e4);
      break;
    }
    break;
  case 42: //改变主地图坐标
