...
void ChessRewardFlow::showNeigongReward()
{
    ...
    for (;;)
    {
        ...
        auto menu = makeIndexedMenu("選擇内功，内功將對所有成員自動生效", menuData, menuConfig, {std::make_shared<NeigongDetailPanel>(detailChoices)});
        menu->run();
...
bool ChessRewardFlow::rewardNeigong(int maxTier)
{
    ...
    menuConfig.x = menuAnchor.x;
    menuConfig.y = menuAnchor.y;
    auto menu = makeIndexedMenu("選擇內功，内功將對所有成員自動生效", menuData, menuConfig, {std::make_shared<NeigongDetailPanel>(detailChoices)});
    menu->run();
    int sel = menu->getResult();
    if (sel >= 0)
    {
...
bool ChessRewardFlow::rewardEquipment(int maxTier, int specificId)
{
    ...
    menuConfig.x = menuAnchor.x;
    menuConfig.y = menuAnchor.y;
    auto menu = makeIndexedMenu("選擇裝備獎勵", menuData, menuConfig, {std::make_shared<EquipmentDetailPanel>(detailEquipments, [](const EquipmentDef&) { return EquipmentDetailState{}; })});
    menu->run();
    int sel = menu->getResult();
    if (sel >= 0)
    {
