...
$avifLibrary = Join-Path $paths.VcpkgWasm 'lib\libavif.a'
$projectOverlayPorts = Join-Path $paths.ProjectDir 'ports'
...
$installCommand = @(
    ...
    "--overlay-ports=`"$projectOverlayPorts`"",
    ...
    'sdl3-image[avif,png,webp]:wasm32-emscripten sdl3-ttf[svg]:wasm32-emscripten',
...
Ensure-PathExists -Path $avifLibrary -Message "WASM AVIF deps not installed. Run from vcpkg root:`n$installCommand"
