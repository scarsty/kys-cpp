...
- PotDll.h（smallpot 的头文件，位于 local/include）
...
音频播放默认使用 SDL3_mixer。MIDI 通过 FluidSynth，MP3 通过 mpg123，WAV 使用 SDL3_mixer 自带解码。

链接选项并未分别处理。MSVC 和 GCC 中，如果某个库的功能并未被用到，即使其包含在链接选项中，也不会参与实质的链接。